...
public interface AccountRepository extends JpaRepository<Account, Long>{
    ...
    @Query("SELECT a FROM Account a WHERE (LOWER(a.firstName) LIKE LOWER(CONCAT('%', :firstName, '%')) " +
            "OR LOWER(a.lastName) LIKE LOWER(CONCAT('%', :lastName, '%'))) " +
            "AND a.id != :accountId " +
            "AND a.id NOT IN (SELECT ac.id FROM Account acc JOIN acc.contacts ac WHERE acc.id = :accountId)")
    Page<Account> findByFirstNameAndLastNameExcludingId(
